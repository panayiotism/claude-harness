...
Major release transforming claude-harness with a 4-layer memory architecture inspired by [Context-Engine](https://github.com/zeddy89/Context-Engine), failure prevention from [Agent-Foreman](https://github.com/mylukin/agent-foreman), and test-driven features from [Anthropic's autonomous-coding quickstart](https://github.com/anthropics/claude-quickstarts/tree/main/autonomous-coding).

### New Features

#### 4-Layer Memory Architecture
- **Working Memory** (`memory/working/`) - Computed fresh each session, contains only relevant context
- **Episodic Memory** (`memory/episodic/`) - Rolling window of 50 recent decisions with rationale
- **Semantic Memory** (`memory/semantic/`) - Persistent project architecture, patterns, and constraints
- **Procedural Memory** (`memory/procedural/`) - Append-only logs of successes and failures

#### Failure Prevention System
- Records every failed approach with root cause analysis
- Before implementation, checks if proposed approach matches past failures
- Warns and suggests alternatives from successful approaches
- Never repeat the same mistake twice

#### Test-Driven Features
- New `/generate-tests` command creates tests BEFORE implementation
- Tests define acceptance criteria upfront
- Implementation loop runs until all tests pass
- Prevents "marking complete without verification"

#### Two-Phase Pattern
- **Phase 1**: `/plan-feature` - Analyze requirements, check impact, review past failures
- **Phase 2**: `/implement` - Execute plan with verification loop
- Clear separation reduces errors and improves outcomes

#### Impact Analysis
- Dependency graph tracks file relationships
- Warns when changes affect high-impact files
...
| Command | Description |
|---------|-------------|
| `/claude-harness:generate-tests` | Generate test cases before implementation (TDD) |
| `/claude-harness:plan-feature` | Plan implementation with impact analysis |
| `/claude-harness:check-approach` | Validate approach against past failures |
...
.claude-harness/
├── memory/
│   ├── working/context.json       # Rebuilt each session
│   ├── episodic/decisions.json    # Rolling 50 decisions
│   ├── semantic/architecture.json # Project patterns
│   └── procedural/
...
├── features/
│   ├── active.json                # Current features
│   └── tests/{id}.json            # Generated test specs
├── impact/
│   └── dependency-graph.json      # File relationships
├── agents/context.json            # Orchestration state
├── loops/state.json               # Agentic loop state
...
Automatic migration when you run `/claude-harness:setup`:
- Creates new memory directory structure
- Migrates `agent-memory.json` → `memory/procedural/`
- Migrates `working-context.json` → `memory/working/`
- Migrates `agent-context.json` → `agents/context.json`
- Preserves `feature-list.json` for backward compatibility
- Creates `.migrated-from-v2` marker file

### Breaking Changes

None. v2.x projects are automatically migrated.

### Upgrade Instructions

```bash
# 1. Update the plugin (pull latest)
# 2. In your project, run:
/claude-harness:setup
# This auto-detects v2.x and migrates to v3.0
```
