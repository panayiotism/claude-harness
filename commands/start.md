...
12. **Check active loop state** (PRIORITY):
   ...
   - Check `autonomous` field to determine if Ralph mode is active
   ...
   - **Read progress.txt** (Ralph-style cross-context log):
     - If `.claude-harness/loops/progress.txt` exists and is non-empty:
       ```
       ┌─────────────────────────────────────────────────────────────────┐
       │  📋 PROGRESS LOG (last 5 attempts)                             │
       ├─────────────────────────────────────────────────────────────────┤
       │  {tail -5 progress.txt}                                        │
       └─────────────────────────────────────────────────────────────────┘
       ```

   - **Read guardrails.md** (failure patterns to avoid):
     - If `.claude-harness/loops/guardrails.md` exists and is non-empty:
       ```
       ┌─────────────────────────────────────────────────────────────────┐
       ...
       └─────────────────────────────────────────────────────────────────┘
       ```
     - These patterns have failed repeatedly - DO NOT repeat them
